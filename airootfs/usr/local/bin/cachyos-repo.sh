...
if [ ! -f $MNT/etc/pacman.conf ]; then
  echo "File [$MNT/etc/pacman.conf] not found!"
  exit 1
fi
...
check_if_repo_was_added() {
    cat $MNT/etc/pacman.conf | grep "(cachyos\|cachyos-v3\|cachyos-testing-v3\|cachyos-v4)" > /dev/null
    echo $?
}

check_if_repo_was_commented() {
    cat $MNT/etc/pacman.conf | grep "cachyos\|cachyos-v3\|cachyos-testing-v3\|cachyos-v4" | grep -v "#\[" | grep "\[" > /dev/null
    echo $?
}
...
add_specific_repo() {
    ...
    local pacman_conf_path_backup="$MNT/etc/pacman.conf.bak"
...
run_install() {
    ...
    pacman -U "${mirror_url}/cachyos-keyring-2-1-any.pkg.tar.zst"        \
              ...
              "${mirror_url}/pacman-6.0.2-10-x86_64.pkg.tar.zst" --noconfirm
