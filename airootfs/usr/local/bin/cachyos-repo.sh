...
run_install() {
    ...
    if [ $is_repo_added -ne 0 ] || [ $is_repo_commented -ne 0 ]; then
        add_specific_repo x86-64-v3 /usr/local/share/cachyos-repo/install-repo.awk cachyos-v3
        add_specific_repo x86-64-v4 /usr/local/share/cachyos-repo/install-v4-repo.awk cachyos-v4
    else
...
run_remove() {
    ...
    if [ $is_repo_added -eq 0 ] || [ $is_repo_commented -eq 0 ]; then
        cp $pacman_conf $pacman_conf_cachyos
        gawk -i inplace -f /usr/local/share/syncopated-repo/remove-repo.awk $pacman_conf_cachyos || true
